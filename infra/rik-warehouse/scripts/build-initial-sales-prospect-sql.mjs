...
const buildSql = (candidates) => {
  ...
  for (const candidate of candidates) {
    ...
    if (
      ...
      prospectCount += 1;
      lines.push(
        `insert into sales_crm.prospect_companies (list_id, registry_code, company_name, legal_form, status, registered_at, company_age_years, address_summary, average_revenue_last_two, latest_employee_count, latest_fiscal_year, vta_signal, sales_signal, priority_score, score_reason, recommended_pitch, next_action, crm_status, source_system, source_observed_at) select ${toSql(listId)}, ${toSql(candidate.registryCode)}, ${toSql(candidate.companyName)}, ${toSql(candidate.legalForm)}, ${toSql(candidate.statusText)}, ${toSql(candidate.registeredAt)}, ${candidate.companyAgeYears}, ${toSql(candidate.addressSummary)}, ${toSqlNumber(candidate.revenue)}, ${toSqlNumber(candidate.employeeCount)}, ${candidate.latestFiscalYear}, 'not_checked', ${toSql(candidate.salesSignal)}, ${candidate.priorityScore}, ${toSqlArray(candidate.scoreReason)}, ${toSql(pitch)}, 'Kontrolli VTA jääk ja märgi Toomasele helistamiseks.', 'call_next', 'rik_avaandmed_csv', ${toSql(nowIso)} where not exists (select 1 from sales_crm.prospect_companies where registry_code = ${toSql(candidate.registryCode)} and crm_status not in ('do_not_contact', 'won', 'lost'));`,
